...
plot_prompt = f"""
    ...
    Write this as one paragraph and make the summarization exciting. This text will be used to promote the launch of a new book. Inculde the title of the book in the summarization.
"""

messages = [
    {"role": "system", "content": "You are a digital marketer working at a small publishing company called Ember Press. Part of your job is to generate promotional content for new book releases. Ember Press publishes young adult novels and is set to release a new thriller called The Forgotten House. You need to prepare content that will be used in emails and social media."},
...
for review in book_reviews:
    reviews_prompt = f"""Determine the sentiment of {review} as a single word, Positive or Negative."""
...
for item in book_reviews_with_sentiments:
    if item["sentiment"] == "Positive":
        positive_reviews += item["review"] + "\n"


email_prompt = f"""Generate an exciting email no more than 100 words that makes people want to run out and buy the book using the book summary, in between < and > <{book_summary}> and a list of postive reviews, in between < and > <{positive_reviews}>."""
